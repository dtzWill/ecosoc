...
bool bSSA::runOnModule(Module &M) {
		...
		errs()<<"\n\nSources of address " <<src.size()<<"\n";
		errs()<<"Sinks of address (public channels) " <<dst.size()<<"\n";
		...
		for (unsigned int i=0; i<src.size(); i++) {
			for (unsigned int j=0; j<dst.size(); j++) {
				...
				if (gIt != gIte) {
					...
					ss << "./subgraphLLVM" << c <<".dot"; c++;
					//ss1 << srcFileName[i].data();
					ss1 << "file " << srcFileName[i].data() << " line " << srcLine[i]<< " to file "<< dstFileName[j].data() << " line " << dstLine[j]<<" ";
...
char bSSA::ID = 0;
static RegisterPass<bSSA> X("flowTracking", "Implicit and Explicit flow analysis");
