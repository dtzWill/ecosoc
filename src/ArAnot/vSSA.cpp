...
static const std::string vSSA_PHI = "phi_node";
static const std::string vSSA_SIG = "sigma_node";

static vSSA::ValueMapTy ValueMap;
static vSSA::SigmaMapTy SigmaMap;
...
Value *vSSA::getSigmaForAt(Value *V, BasicBlock *BB) {
  DEBUG(dbgs() << "getSigmaForAt: " << *V << " at " << BB->getName() << "\n");
  if (CastInst *CI = dyn_cast<CastInst>(V))
    return getSigmaForAt(CI->getOperand(0), BB);
  if (ValueMap.count(V))
    V = ValueMap[V];
  if (!SigmaMap.count(V)) {
    DEBUG(dbgs() << "!Count\n");
  ...
  if (!SigmaMap[V]->count(BB)) {
    DEBUG(dbgs() << "!Count2\n");
    return V;
  }
  return (*SigmaMap[V])[BB];
}
...
void vSSA::createSigmasIfNeeded(BasicBlock *BB)
{
  ...
  //  return;
  //DEBUG(dbgs() << "createSigmasIfNeeded: " << BB->getName() << "\n");
	...
	*/
	...
	if ((bi = dyn_cast<BranchInst>(ti))) {
        ...
        DEBUG(dbgs() << "comparison: " << *comparison << "\n");
				for (User::op_iterator opit = comparison->op_begin(), opend = comparison->op_end(); opit != opend; ++opit) {
					Value *operand = *opit;
          DEBUG(dbgs() << "operand: " << *operand << "\n");
        }
...
{
  DEBUG(dbgs() << "renaming uses of: " << *V << " to " << *phi << "\n");
  ...
  DEBUG(dbgs() << "Creating sigmas for: " << phi->getParent()->getName() << "\n");
...
char vSSA::ID = 0;
/* INITIALIZE_PASS_BEGIN(vSSA, "vssa", "Static Single Assignment Construction", true, true)
INITIALIZE_PASS_DEPENDENCY(BreakCriticalEdges)
INITIALIZE_PASS_DEPENDENCY(PromotePass)
INITIALIZE_PASS_DEPENDENCY(DominatorTree)
INITIALIZE_PASS_DEPENDENCY(DominanceFrontier)
INITIALIZE_PASS_END(vSSA, "vssa", "Static Single Assignment Construction", true, true)
*/
