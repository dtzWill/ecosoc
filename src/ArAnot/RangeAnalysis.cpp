...
const std::string sigmaString = "sigma_node";
...
IntraProceduralRA<CGT>::~IntraProceduralRA(){
	...
	maxVisit = maxtimes;
#endif
	for (VarNodes::iterator vit = CG->getVars()->begin(), vend = CG->getVars()->end(); vit != vend; ++vit) {
		Range RG = vit->second->getRange(); 
		if (!RG.isUnknown()) {
			DEBUG(dbgs() << "RANGE:" << *vit->second->getValue() << ": ");
			DEBUG(RG.print(dbgs()));
			DEBUG(dbgs() << "\n");
		}
	}
...
static RegisterPass<IntraProceduralRACousot> X("ra-intra-cousot", "Cousot's intra-procedural range analysis");
//INITIALIZE_PASS_BEGIN(IntraProceduralRACousot, "ra-intra-cousot", "Cousot's intra-procedural range analysis", true, true)
//INITIALIZE_PASS_DEPENDENCY(vSSA)
//INITIALIZE_PASS_END(IntraProceduralRACousot, 

//FunctionPass *llvm::createIntraProceduralRACousotFunctionPass() {
//  return new IntraProceduralRACousot(); 
//}

template<class CGT>
char InterProceduralRA<CGT>::ID = 0;
static RegisterPass<InterProceduralRACousot> Y("ra-inter-cousot", "Cousot's inter-procedural range analysis");
//INITIALIZE_PASS_BEGIN(InterProceduralRACousot, "ra-inter-cousot", "Cousot's inter-procedural range analysis", true, true)
//INITIALIZE_PASS_DEPENDENCY(vSSA)
//INITIALIZE_PASS_END(InterProceduralRACousot, "ra-inter-cousot", "Cousot's inter-procedural range analysis", true, true)

//ModulePass *llvm::createInterProceduralRACousotModulePass() {
//  return new InterProceduralRACousot(); 
//}
...
void ConstraintGraph::buildValueSwitchMap(const SwitchInst *sw) {
	...
	// Handle the rest of cases
        for (SwitchInst::ConstCaseIt i = sw->case_begin(), e = sw->case_end(); i != e; ++i) {
		const BasicBlock *succ = i.getCaseSuccessor(); 
		// dbgs() << "const BasicBlock *succ = " << *i.getCaseSuccessor() << ";\n";
		/* BasicBlock *succ = sw->getSuccessor(i); */
		/* Modified. */
		/* const ConstantInt *constant = sw->getCaseValue(i); */
		/* const ConstantInt *constant = dyn_cast<ConstantInt*>(sw->getOperand(i * 2)); */
		/* ConstantInt *constant = ((SwitchInst*)sw)->findCaseDest(succ); */
		const ConstantInt *constant = i.getCaseValue();
		// dbgs() << "const ConstantInt *constant = " << *i.getCaseValue() << ";\n";
...
bool Meet::narrow(BasicOp* op, const SmallVector<APInt, 2> *constantvector) {
	...
	// dbgs() << "NARROW::" << op->getSink()->getValue()->getName() << ": " << Range(oLower,oUpper) << " -> " << op->getSink()->getRange() << "\n";
	return hasChanged;
}
...
void ConstraintGraph::update(const UseMap &compUseMap,
	...
	/* Breaks during narrowing. */
	/* dbgs() << "actv = " << "{ ";
	for (SmallPtrSetIterator<const Value*> i = actv.begin(), e = actv.end(); i != e; ++i) { 
		dbgs() << " " << **i;
	}
	dbgs() << "};\n";
	*/
	while (!actv.empty()) {
		...
		/* dbgs() << "compUseMap = " << "{ ";
		for (SmallPtrSetIterator<BasicOp*> i = L.begin(), e = L.end(); i != e; ++i) { 
			dbgs() << " " << *(*i)->getSink()->getValue();
		}
		dbgs() << "};\n";
		*/
		SmallPtrSetIterator<BasicOp*> bgn = L.begin(), end = L.end();
		...
		for (; bgn != end; ++bgn) {
			if (meet(*bgn, &constantvector)) {
				...
				// keep an order or insertions and removals.
               			// dbgs() << "actv.insert(" << *(*bgn)->getSink()->getValue() << ");\n";
