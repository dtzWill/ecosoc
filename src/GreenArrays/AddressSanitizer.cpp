...
static cl::opt<bool> ClInstrumentReads("ga-asan-instrument-reads",
       cl::desc("instrument read instructions"), cl::Hidden, cl::init(true));
static cl::opt<bool> ClInstrumentWrites("ga-asan-instrument-writes",
       cl::desc("instrument write instructions"), cl::Hidden, cl::init(true));
static cl::opt<bool> ClInstrumentAtomics("ga-asan-instrument-atomics",
       cl::desc("instrument atomic instructions (rmw, cmpxchg)"),
       cl::Hidden, cl::init(true));
static cl::opt<bool> ClAlwaysSlowPath("ga-asan-always-slow-path",
       cl::desc("use instrumentation with slow path for all accesses"),
       cl::Hidden, cl::init(false));
static cl::opt<bool> ClOptASI("ga-asan-asi",
       cl::desc("Optimize instrumentation with ASI"), cl::Hidden, cl::init(false));
// This flag limits the number of instructions to be instrumented
// in any given BB. Normally, this should be set to unlimited (INT_MAX),
// but due to http://llvm.org/bugs/show_bug.cgi?id=12652 we temporary
// set it to 10000.
static cl::opt<int> ClMaxInsnsToInstrumentPerBB("ga-asan-max-ins-per-bb",
       cl::init(10000),
       cl::desc("maximal number of instructions to instrument in any given BB"),
       cl::Hidden);
// This flag may need to be replaced with -f[no]asan-stack.
static cl::opt<bool> ClStack("ga-asan-stack",
       cl::desc("Handle stack memory"), cl::Hidden, cl::init(true));
// This flag may need to be replaced with -f[no]asan-use-after-return.
static cl::opt<bool> ClUseAfterReturn("ga-asan-use-after-return",
       cl::desc("Check return-after-free"), cl::Hidden, cl::init(false));
// This flag may need to be replaced with -f[no]asan-globals.
static cl::opt<bool> ClGlobals("ga-asan-globals",
       cl::desc("Handle global objects"), cl::Hidden, cl::init(true));
static cl::opt<bool> ClInitializers("ga-asan-initialization-order",
       cl::desc("Handle C++ initializer order"), cl::Hidden, cl::init(false));
static cl::opt<bool> ClMemIntrin("ga-asan-memintrin",
       cl::desc("Handle memset/memcpy/memmove"), cl::Hidden, cl::init(true));
static cl::opt<bool> ClRealignStack("ga-asan-realign-stack",
       cl::desc("Realign stack to 32"), cl::Hidden, cl::init(true));
static cl::opt<std::string> ClBlacklistFile("ga-asan-blacklist",
...
static cl::opt<bool> ClKeepUninstrumented("ga-asan-keep-uninstrumented-functions",
...
static cl::opt<int> ClMappingScale("ga-asan-mapping-scale",
       cl::desc("scale of asan shadow mapping"), cl::Hidden, cl::init(0));
static cl::opt<int> ClMappingOffsetLog("ga-asan-mapping-offset-log",
       cl::desc("offset of asan shadow mapping"), cl::Hidden, cl::init(-1));
static cl::opt<bool> ClShort64BitOffset("ga-asan-short-64bit-mapping-offset",
       cl::desc("Use short immediate constant as the mapping offset for 64bit"),
       cl::Hidden, cl::init(true));

// Optimization flags. Not user visible, used mostly for testing
// and benchmarking the tool.
static cl::opt<bool> ClOpt("ga-asan-opt",
       cl::desc("Optimize instrumentation"), cl::Hidden, cl::init(true));
static cl::opt<bool> ClOptSameTemp("ga-asan-opt-same-temp",
       cl::desc("Instrument the same temp just once"), cl::Hidden,
       cl::init(true));
static cl::opt<bool> ClOptGlobals("ga-asan-opt-globals",
       cl::desc("Don't instrument scalar globals"), cl::Hidden, cl::init(true));

static cl::opt<bool> ClCheckLifetime("ga-asan-check-lifetime",
       cl::desc("Use llvm.lifetime intrinsics to insert extra checks"),
       cl::Hidden, cl::init(false));

// Debug flags.
static cl::opt<int> ClDebug("ga-asan-debug", cl::desc("debug"), cl::Hidden,
                            cl::init(0));
static cl::opt<int> ClDebugStack("ga-asan-debug-stack", cl::desc("debug stack"),
                                 cl::Hidden, cl::init(0));
static cl::opt<std::string> ClDebugFunc("ga-asan-debug-func",
                                        cl::Hidden, cl::desc("Debug func"));
static cl::opt<int> ClDebugMin("ga-asan-debug-min", cl::desc("Debug min inst"),
                               cl::Hidden, cl::init(-1));
static cl::opt<int> ClDebugMax("ga-asan-debug-max", cl::desc("Debug man inst"),
                               cl::Hidden, cl::init(-1));
...
static RegisterPass<AddressSanitizer> X("ga-asan",
    "AddressSanitizer: detects use-after-free and out-of-bounds bugs.",
    false, false);

char AddressSanitizerModule::ID = 0;
static RegisterPass<AddressSanitizerModule> Y("ga-asan-module",
    "AddressSanitizer: detects use-after-free and out-of-bounds bugs."
...
void AddressSanitizer::instrumentAddress(Instruction *OrigIns,
  ...
  if (ClOptASI) {
    //errs() << "Addr: " << *Addr << "\n";
    if (OrigIns->getMetadata("memsafe") || OrigIns->getMetadata("safe")) {
      //errs() << *I << " is safe\n";
      OrigIns->setMetadata("noinstrument", MDNode::get(*C,  ArrayRef<Value*>()));
      Safe++;
      return;
    }
  ...
  /* if (ClOptASI) {
    bool AllUsesSafe = true;
    for (Value::use_iterator VI = OrigIns->use_begin(), VE = OrigIns->use_end();
        VI != VE; ++VI) {
      if (Instruction *I = dyn_cast<Instruction>(*VI)) {
        if (!I->getMetadata("safe"))
          AllUsesSafe = false; 
      } else {
        AllUsesSafe = false; 
      }
...
bool AddressSanitizerModule::runOnModule(Module &M) {
  ...
  dbgs() << "================ ASAN STATS ===============\n";
  dbgs() << "==== Number of removed bcs:     " << Safe << "\n";
  dbgs() << "==== Number of inserted bcs:    " << NotSafe << "\n";
