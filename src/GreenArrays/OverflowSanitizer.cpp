...
STATISTIC(NumTainted, "Number of tainted graph nodes");
STATISTIC(NumClean, "Number of clean graph nodes");
...
bool OverflowSanitizer::isValidInst(Instruction *I) {
	if (dyn_cast<OverflowingBinaryOperator> (I))
		return I->getType()->isIntegerTy()
				&& I->getOperand(0)->getType()->isIntegerTy() && I->getOperand(
...
BasicBlock* OverflowSanitizer::newOverflowOccurrenceBlock(Instruction* I,
		BasicBlock* NextBlock, Value *messagePtr) {
	...
	BasicBlock* result = BasicBlock::Create(*context, "",
			I->getParent()->getParent(), NextBlock);
...
int llvm::OverflowSanitizer::countInstructions() {
	...
	for (Module::iterator Fit = module->begin(), Fend = module->end(); Fit
			!= Fend; Fit++) {
...
int llvm::OverflowSanitizer::countOverflowableInsts() {
	...
	for (Module::iterator Fit = module->begin(), Fend = module->end(); Fit
		...
		for (Function::iterator BBit = Fit->begin(), BBend = Fit->end(); BBit
				!= BBend; BBit++) {
			for (BasicBlock::iterator I = BBit->begin(), E = BBit->end(); I
					!= E; I++) {
				if (isValidInst(I))
					result++;
			}
...
bool ::OverflowSanitizer::runOnModule(Module& M) {
	...
	std::set<Value*> memAllocsOrInd; //keep track of memory allocation sizes and indices
	//	Graph* graph = depGraph.depGraph;
		...
		for (Function::iterator BB = F->begin(), endBB = F->end(); BB != endBB; ++BB) {
			for (BasicBlock::iterator I = BB->begin(), endI = BB->end(); I
				...
				if (CallInst *CI = dyn_cast<CallInst> (I)) {
					...
					if (FF) {
						StringRef name = FF->getName();
						if (name == "malloc" || name == "realloc" || name
								== "calloc" || name == "xalloc" || name
								== "Znwj" || name == "ZnwjRKSt9nothrow_t"
								|| name == "Znwm" || name
								== "ZnwmRKSt9nothrow_t" || name == "Znaj"
								|| name == "ZnajRKSt9nothrow_t" || name
								== "Znam" || name == "ZnamRKSt9nothrow_t") {
							memAllocsOrInd.insert(CI);
						}
					}
				} else if (GetElementPtrInst *GEP = dyn_cast<GetElementPtrInst> (I)) {
					for (unsigned Idx = 1; Idx < GEP->getNumOperands(); ++Idx) {
						memAllocsOrInd.insert(GEP->getOperand(Idx));
					}
				}
			}
		}
	...
	std::set<GraphNode*> depValues = graph->getDepValues(memAllocsOrInd, false);

	for (std::set<GraphNode*>::iterator it = depValues.begin(), e =
			depValues.end(); it != e; ++it) {
		if (VarNode * VN = dyn_cast<VarNode> (*it)) {
			Value* v = VN->getValue();
			if (Instruction *inst = dyn_cast<Instruction>(v)) {
				DEBUG(errs() << "Inserted value in valuesToSafe\n");
				if (isValidInst(inst)) {
					valuesToSafe.insert(inst);
				}
			}
		}
	}
	...
	for (std::set<Instruction*>::iterator i = valuesToSafe.begin(), e =
			valuesToSafe.end(); i != e; i++) {
...
void OverflowSanitizer::markAsNotOriginal(Instruction& inst) {
	inst.setMetadata("new-inst",
			MDNode::get(*context, llvm::ArrayRef<Value*>()));
}
...
void OverflowSanitizer::insertGlobalDeclarations() {
	...
	std::string
			message3("(Suspected) Overflow occurred in %s, line %d. [%d]\n");
	std::string
			message4(
					"(Suspected) Truncation with data loss occurred in %s, line %d. [%d]\n");

	Constant* stringConstant = strToLLVMConstant(message1);
	GlobalVariable* messageStr = new GlobalVariable(*module,
			stringConstant->getType(), true,
			llvm::GlobalValue::InternalLinkage, stringConstant,
			"OverflowMessage");

	//Get the int8ptr to our message
	constZero = ConstantInt::get(Type::getInt32Ty(*context), 0);
	Constant* constArray = ConstantExpr::getInBoundsGetElementPtr(messageStr,
			constZero);
	overflowMessagePtr = ConstantExpr::getBitCast(constArray,
	...
	truncErrorMessagePtr = ConstantExpr::getBitCast(constArray,
			PointerType::getUnqual(Type::getInt8Ty(*context)));
	...
	//Get the int8ptr to our message
	constArray = ConstantExpr::getInBoundsGetElementPtr(messageStr, constZero);
	overflowMessagePtr2 = ConstantExpr::getBitCast(constArray,
			PointerType::getUnqual(Type::getInt8Ty(*context)));
	...
	//Get the int8ptr to our message
	constArray = ConstantExpr::getInBoundsGetElementPtr(messageStr, constZero);
	truncErrorMessagePtr2 = ConstantExpr::getBitCast(constArray,
			PointerType::getUnqual(Type::getInt8Ty(*context)));
	...
	if (GVstderr == NULL) {
		...
		StructType* IO_FILE_ty =
				StructType::create(*context, "struct._IO_FILE");
	...
	FPrintF = module->getOrInsertFunction("fprintf",
			FunctionType::get(Type::getVoidTy(*context), aRParams, true));
	...
	if (!alreadyDeclared) {
		AbortF = Function::Create(AbortFTy, GlobalValue::ExternalLinkage,
				"abort", module);
	}
...
void OverflowSanitizer::insertInstrumentation(Instruction* I,
		BasicBlock* AbortBB, OvfPrediction Pred) {
	...
	Value* messagePtr = (Pred == OvUnknown ? overflowMessagePtr
			: overflowMessagePtr2);
	...
	case Instruction::Add:
		...
		if (isSigned) {
			...
			positiveOp1 = new ICmpInst(nextInstruction, CmpInst::ICMP_SGE, op1,
					constZero);
			positiveOp2 = new ICmpInst(nextInstruction, CmpInst::ICMP_SGE, op2,
					constZero);
			canCauseOverflow = BinaryOperator::Create(Instruction::And,
					positiveOp1, positiveOp2, "", nextInstruction);
			negativeResult = new ICmpInst(nextInstruction, CmpInst::ICMP_SLT,
					I, constZero);
			hasIntegerBug1 = BinaryOperator::Create(Instruction::And,
					canCauseOverflow, negativeResult, "", nextInstruction);

			//	or	add two big negative numbers and get a positive number as result
			negativeOp1 = new ICmpInst(nextInstruction, CmpInst::ICMP_SLT, op1,
					constZero);
			negativeOp2 = new ICmpInst(nextInstruction, CmpInst::ICMP_SLT, op2,
					constZero);
			canCauseOverflow = BinaryOperator::Create(Instruction::And,
					negativeOp1, negativeOp2, "", nextInstruction);
			positiveResult = new ICmpInst(nextInstruction, CmpInst::ICMP_SGE,
					I, constZero);
			hasIntegerBug2 = BinaryOperator::Create(Instruction::And,
					canCauseOverflow, positiveResult, "", nextInstruction);

			// AND the results
			hasIntegerBug = BinaryOperator::Create(Instruction::Or,
					hasIntegerBug1, hasIntegerBug2, "", nextInstruction);
		} else {
			...
			hasIntegerBug = BinaryOperator::Create(Instruction::Or,
					lessThanOp1, lessThanOp2, "", nextInstruction);
		}
		break;
	...
	case Instruction::Sub:
		...
		if (isSigned) {
			...
			positiveOp2 = new ICmpInst(nextInstruction, CmpInst::ICMP_SGE, op2,
					constZero);
			canCauseOverflow = BinaryOperator::Create(Instruction::And,
					negativeOp1, positiveOp2, "", nextInstruction);
			positiveResult = new ICmpInst(nextInstruction, CmpInst::ICMP_SGE,
					I, constZero);
			hasIntegerBug1 = BinaryOperator::Create(Instruction::And,
					canCauseOverflow, positiveResult, "", nextInstruction);
			...
			negativeOp2 = new ICmpInst(nextInstruction, CmpInst::ICMP_SLT, op2,
					constZero);
			canCauseOverflow = BinaryOperator::Create(Instruction::And,
					positiveOp1, negativeOp2, "", nextInstruction);
			negativeResult = new ICmpInst(nextInstruction, CmpInst::ICMP_SLT,
					I, constZero);
			hasIntegerBug2 = BinaryOperator::Create(Instruction::And,
					canCauseOverflow, negativeResult, "", nextInstruction);
			...
			hasIntegerBug = new ICmpInst(nextInstruction, CmpInst::ICMP_ULT,
					op1, op2);
	...
	case Instruction::Mul:
		...
		canCauseOverflow = new ICmpInst(nextInstruction, CmpInst::ICMP_NE, op1,
				constZero);
		...
		newBB2 = BasicBlock::Create(*context, "", I->getParent()->getParent(),
				newBB);
		...
		if (isSigned) {
			...
			tmpValue = BinaryOperator::Create(Instruction::SDiv, I, op1, "",
					branch);
			hasIntegerBug = new ICmpInst(branch, CmpInst::ICMP_NE, tmpValue,
					op2);

		} else {

			tmpValue = BinaryOperator::Create(Instruction::UDiv, I, op1, "",
					branch);
			hasIntegerBug = new ICmpInst(branch, CmpInst::ICMP_NE, tmpValue,
					op2);
	...
	case Instruction::Shl:
		...
		if (isSigned) {
			...
			negativeResult = new ICmpInst(nextInstruction, CmpInst::ICMP_SLT,
					I, constZero);
			hasIntegerBug1 = BinaryOperator::Create(Instruction::And,
					positiveOp1, negativeResult, "", nextInstruction);

			negativeOp1 = new ICmpInst(nextInstruction, CmpInst::ICMP_SLT, op1,
					constZero);
			canCauseOverflow = new ICmpInst(nextInstruction, CmpInst::ICMP_NE,
					op2, constZero);
			hasIntegerBug1 = BinaryOperator::Create(Instruction::And,
					canCauseOverflow, negativeOp1, "", nextInstruction);
			...
			hasIntegerBug = new ICmpInst(nextInstruction, CmpInst::ICMP_ULT, I,
					op1);
	...
	case Instruction::Trunc:
		 ...
		 */
		messagePtr = (Pred == OvUnknown ? truncErrorMessagePtr
				: truncErrorMessagePtr2);
		if (isSigned) {

			tmpValue = new SExtInst(I, op1->getType(), "", nextInstruction);
			hasIntegerBug = new ICmpInst(nextInstruction, CmpInst::ICMP_NE,
					tmpValue, op1);
	...
	if (I->getOpcode() != Instruction::Mul) {
		...
		branch = BranchInst::Create(
				newOverflowOccurrenceBlock(I, NextBB, messagePtr), newBB,
				hasIntegerBug, I->getParent());
...
Constant* OverflowSanitizer::getSourceFile(Instruction* I) {
	...
	if (SourceFiles.count(File)) {
		...
		GlobalVariable* sourceFileStr = new GlobalVariable(*module,
				stringConstant->getType(), true,
				llvm::GlobalValue::InternalLinkage, stringConstant,
				"SourceFile");
		...
		//Get the int8ptr to our message
		Constant* constArray = ConstantExpr::getInBoundsGetElementPtr(
				sourceFileStr, constZero);
		Constant* sourceFilePtr = ConstantExpr::getBitCast(constArray,
				PointerType::getUnqual(Type::getInt8Ty(*context)));
...
void OverflowSanitizer::getAnalysisUsage(AnalysisUsage &AU) const {
	AU.addRequired<bSSA> ();
