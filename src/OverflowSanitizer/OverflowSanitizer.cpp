...
bool ::OverflowSanitizer::runOnModule(Module& M) {
		...
		for (Function::iterator BB = F->begin(), endBB = F->end(); BB != endBB; ++BB) {
			for (BasicBlock::iterator I = BB->begin(), endI = BB->end(); I
				...
				if (CI) {
					...
					if (FF && (FF->getName() == "malloc" || FF->getName()
							== "realloc" || FF->getName() == "calloc"
							|| FF->getName() == "xalloc")) {
						memAllocs.insert(CI);
					}
				}
	...
	std::set<Value*> depValues = graph->getDepValues(memAllocs, false);

	for (std::set<Value*>::iterator it = depValues.begin(), e = depValues.end(); it
			!= e; ++it) {
